...
favicon = "favicon.png"
st.set_page_config(
    page_title="3G Data Processing Application",
    page_icon=favicon,
    layout="wide"
)
...
with st.sidebar:
    st.markdown(
        "<h2 style='text-align:center; color:#660a93;'>3G Reporting</h2>",
        unsafe_allow_html=True
    )
...
if selected == "About":
    ...
    st.write(
        "This 3G Data Processing tool automates **Day & Hour level KPI aggregation** "
...
# ===================== TOOL =====================
if selected == "Tool":
    st.markdown("## 📊 3G Data Processing Application")
    ...
    # -------- DAY CELL LEVEL --------
    if sheet_type == "BBH (Cell Day)" and "WCEL name" in df.columns:
        pivot = pd.pivot_table(
            df,
            index=['WBTS name', 'WCEL name'],
            columns='Date',
        ...
        # -------- CONTINUE MODE --------
    elif sheet_type == "Continue (Hour / Day)" and "WCEL name" in df.columns:
